...
- [ ] 记录审计起点 Git commit、分支和工作区状态。
- [ ] 记录 SQLite 文件 SHA-256、卡牌数量和 SVA 数据源快照信息。
- [ ] 记录 RuleBook、Clause Audit、Token Audit、Ability Audit 和 Catalog 哈希。
- [ ] 记录八套固定卡组的名称、职业、40 张实体列表和 deck SHA-256。
- [ ] 生成八套卡组的可收集卡并集，确认不同卡牌数量。
- [ ] 从卡牌引用、召唤、加入手牌、变身、融合和纹章/信仰定义递归计算
  衍生卡闭包。
- [ ] 检查闭包中的每个卡牌 ID 都能在数据库和 RuleBook 中解析。
- [ ] 将固定基线写成机器可读文件，并为生成逻辑增加确定性测试。
...
- [ ] 为每张卡生成基础字段：ID、名称、职业、类型、所属卡组、数量和引用关系。
- [ ] 为每条主卡文字和替代模式文字生成独立 clause 行，不用一个
  `covered_exact` 布尔值代表整张卡。
- [ ] 每条 clause 记录对应的结构化触发、条件、目标和操作。
- [ ] 每条 clause 记录直接测试、机制测试、官方来源和最后验证提交。
- [ ] 每张卡分别记录以下状态：
  - [ ] 原文映射
  - [ ] 正常路径
  - [ ] 费用/阈值边界
  ...
  - [ ] 区域与容量
  - [ ] 随机性与确定性
  - [ ] command/action mask 一致性
  - [ ] 运行时实际覆盖
  - [ ] 官方裁定或客户端复现
- [ ] 报告必须把“未适用”“已通过”“未测试”“规则不确定”和“已失败”
  分开表示。
- [ ] 为报告的稳定排序、字段完整性、断链测试引用和过期 source hash
  增加自动化测试。
...
- [ ] 将数据库中的中文、英文、日文主文字及替代模式文本与当前规则条款对齐。
- [ ] 检查费用、攻击、生命、类型、职业、特性和基础关键词。
- [ ] 检查卡牌引用是否指向正确实体，而不是同名或错误版本。
- [ ] 检查每个“若”“每当”“直到”“本回合”“自己的回合”等条件是否有
  明确的取值时刻和持续时间。
- [ ] 检查“随机”“全部”“一个”“其他”“不同名称”等数量和去重语义。
- [ ] 将规则有歧义的卡加入待确认队列，不按现有实现自行判定。
- [ ] 对歧义项按以下证据顺序确认：官方 Q&A/规则说明、客户端可重复复现、
  多个可靠录像或测试、单个攻略来源、纯文字推断。
- [ ] 为每个外部裁定保存 URL、访问日期、结论和不超过必要长度的摘要。
- [ ] 官方文字或数据库刷新后，确保旧 source hash 自动失效。
...
- [ ] 建立进入方式矩阵：正常打出、替代模式、直接召唤、从牌库召唤、
  复活、复制、变身、普通进化、超进化和效果进化。
- [ ] 对疾驰、突进、守护、潜行、屏障、必杀、攻击次数、不可攻击、
  不可指定等关键词逐项记录来源。
- [ ] 区分卡牌固有关键词、条件关键词、进化后关键词和临时获得关键词。
- [ ] 验证未满足条件时不提前授予关键词。
- [ ] 验证通过不同进入方式时是否应触发入场曲，以及是否继承实体修正。
- [ ] 验证返回手牌、变身、沉默和离场后应清除或保留的动态能力。
- [ ] 验证普通进化、超进化、“进化时”和“本随从进化时”的触发范围。
- [ ] 验证疾驰/突进随从的当回合主战者和随从攻击目标。
- [ ] 为佐伊未爆能获得疾驰问题保留真实卡牌回归测试。
- [ ] 将关键词矩阵同时接入 RL 攻击动作掩码检查。
