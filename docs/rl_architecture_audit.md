...
- AEC/Gym、单进程与多进程PPO采样、固定评估和对局脚本现默认使用
  `match_setup="official"`，实际经历固定种子随机先后手和双方换牌。实验版本、
  轨迹和检查点记录该预设；缺少字段的旧schema-v2检查点按`legacy`恢复。
  随机自玩验收报告统计先手分布、换牌完成/替换张数、额外PP使用、非法动作、
  mask偏差和截断，并可在任一门槛失败时返回非零退出码；
- 固定种子7的1000局严格验收全部完成换牌，先手分布499/501，执行2000次
...
- 第二十批新增随从属性降低事件、永久授予回合末能力、牌组随从物理属性修正、
  信仰额外模式选择、不可重复随机分支历史、双方战场随机随从目标、有符号
  `negate` 表达式，以及随从出牌时入场曲/爆能强化抑制和自动进化纹章被动。
  6张真实卡通过通用 command、event、condition、target、effect、pending-choice
  与 action-mask 边界接入，将训练目录提升至719张 exact 卡且 supported missing
  归零；手牌当前攻防、随从授予能力/随机历史、信仰模式加成及纹章随机历史进入
  公开决策状态，正式 schema 更新为 `observation-v3.5`，v1继续保持294维且
  111个动作编号不变；
- 第二十一批以2026-07-25官方英/日/简中卡牌库和本地原始JSON为证据，
  用严格 `vanilla_cards` 声明补齐2张无印刷能力随从，并用既有固有能力注册
  补齐9张守护、疾驰、突进或潜行随从。训练目录提升至730张 exact 卡；
  直接测试覆盖全部真实属性、守护/疾驰/突进/潜行合法性、空目标、场面容量、
