...
- `TrainableCardCatalog` 以 `rule_coverage.json` 的 `covered_exact` 为准，
  当前纳入 617 张可收集卡；全部 826 张定义在 worker 启动时进入只读内存，
  对局解析不再访问 SQLite；
...
| 明确不支持 | 102 张缺少 per-card 结构化规则的可收集卡、16 张文本不明确卡 | 不进入 exact 训练目录，不得视为已支持；规则覆盖与 RL 平台状态继续分开报告 |
...
102 张缺规则卡和 16 张文本不明确卡，它们必须继续保持 unsupported 可见性。
