...
- Tests: `python -m unittest discover -s tests -v` currently runs 1170 tests.
...
- Coverage now has a backward-compatible clause audit. The legacy summary still
  reports 87 exact collectible cards, but only 25 have an explicit exact text
  mapping plus direct test evidence; 62 are flagged `unverified_exact` and may
