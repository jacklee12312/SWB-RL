...
- Exact file
  `data/rules/real_fusion_combo_cost_repose_sixteenth_batch.json` closes five
  collectibles (`10324110`, `10514110`, `10332210`, `10342110`, and
  `10364110`), bringing collectible exact coverage to 698/735 (94.97%).
  ...
  safe bound hand-target continuation implement the shared mechanics without
  card-ID branches. Direct tests cover duplicate material names, Combo including
  the current play, empty enemy decks without RNG consumption, hidden exact
  copies and fresh modifier IDs, Engage no/stale/illegal choices, source
  departure, hand capacity, stacked crests, seeded filtered targeting,
...
- Exact file
  ...
  unchanged.
- Exact file
  `data/rules/real_destroyed_amulet_bulk_zone_eighteenth_batch.json` closes
  five collectibles (`10162130`, `10663210`, `10664110`, `10543110`, and
  `10554120`), bringing collectible exact coverage to 708/735 (96.33%).
  Generic persistent destroyed-amulet history, highest-base-cost and
  distinct-name history sampling, filtered bulk deck banish with
  actual-count follow-ups, whole-hand redraw, and simultaneous
  enemy-follower/leader damage implement the shared mechanics without card-ID
  branches. Direct tests cover normal, no-candidate, stale and illegal target
  paths, source departure, hand/board capacity, current-cost deck modifiers,
  simultaneous state checks, seeded replay, multilingual/reference/raw-source
  hashes, Clause/Token consistency, and command/action-mask parity.
  Observation `observation-v3.3` and all 111 action IDs remain unchanged.
- Exact file
  `data/rules/real_listener_context_leader_runtime_nineteenth_batch.json`
  closes five collectibles (`10362210`, `10444120`, `10503210`, `10604110`,
  and `10703210`), bringing collectible exact coverage to 713/735 (97.01%).
  Generic frozen listener event-base-cost and activation-count context, public
  leader Barrier charges, ordered positive-damage replacement, and source
  Fanfare replay implement the shared mechanics without card-ID branches.
  Direct tests cover normal, no-target, source-leave, hand-capacity, illegal
  and stale choice paths, Barrier/replacement priority, countdown and duration
  boundaries, persistent three-step listener order, bounded seeded recursion,
  multilingual/reference/raw-source hashes, Clause/Token consistency, and
