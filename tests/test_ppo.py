...
class PPOTrainerTests(unittest.TestCase):
    ...
    def test_learner_amp_rejects_cpu_device(self) -> None:
        trainer = self.make_trainer()
        try:
            with self.assertRaisesRegex(
                ...
                trainer.configure_experimental_learner_amp(
                    torch.float16
                )
            trainer.configure_experimental_learner_amp(None)
            self.assertIsNone(trainer._learner_amp_dtype)
            self.assertIsNone(trainer._learner_grad_scaler)
        finally:
            trainer.close()

    @unittest.skipUnless(torch.cuda.is_available(), "CUDA is unavailable")
    def test_experimental_learner_amp_update_is_finite(self) -> None:
        trainer = PPOTrainer(
            self.snapshot,
            master_seed=792,
            config=PPOConfig(
        ...
        try:
            ...
            trainer.configure_experimental_learner_amp(torch.float16)
            records, bootstrap, _ = trainer.collect_rollout()
            metrics = trainer.update(records, bootstrap)
            self.assertTrue(
                all(math.isfinite(value) for value in metrics.values())
            ...
            self.assertIsNotNone(trainer._learner_grad_scaler)
        finally:
            trainer.close()
